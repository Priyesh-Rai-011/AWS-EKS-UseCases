...
resource "aws_eip" "nat_gateway" {
  ...
  tags = merge(var.common_tags, { Name = "${var.name}-nat-eip-${count.index + 1}" })
}
...
resource "aws_route_table" "private" {
  ...
  tags = merge(var.common_tags, { Name = "${var.name}-private-rt-${count.index + 1}" })
...
resource "aws_route_table" "database" {
  ...
  tags = merge(var.common_tags, { Name = "${var.name}-database-rt" })
}
