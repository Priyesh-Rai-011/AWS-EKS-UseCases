...
# Trust policy uses Federated OIDC issuer instead of pods.eks.amazonaws.com.
# The sub condition pins this role to the exact service account in kube-system.
# ==============================================================================
...
# ==============================================================================
# ROLE 4 — UMS APP ROLE  (IRSA)
# Trust policy uses Federated OIDC issuer instead of pods.eks.amazonaws.com.
# The sub condition pins this role to ums-app-sa in the ums-app namespace.
# No aws_eks_pod_identity_association needed — the SA annotation does the wiring.
# ==============================================================================
resource "aws_iam_role" "ums_app_role" {
  name        = "${var.cluster_name}-ums-app-role"
  description = "Assumed by ums-app pods via IRSA to read DB credentials from Secrets Manager"
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:ums-app:ums-app-sa"
          "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:aud" = "sts.amazonaws.com"
        }
      }
    }]
